...
class Trafo39Step(base.AugmentationStep):
    ...
    def do_augment(self, doc2: model.Document):
        ...
        entity_mentions_by_type = Trafo39Step.extract_entity_mentions_by_type(
            self, token_sequences, tag_sequences
        )
        ...
        for sentence in doc.sentences:
            changes_counter = 0
            assert len(token_sequences[sentence_idx]) == len(
                tag_sequences[sentence_idx]
            ), f"token_sequence and tag_sequence should have same length! {len(token_sequences[sentence_idx])}!={len(tag_sequences[sentence_idx])}"

            # get the entities of the sentence
            entities = Trafo39Step.extract_entities(
                self, token_sequences[sentence_idx], tag_sequences[sentence_idx]
            )
            ...
            if len(entities) == 0:
                ...
                for entity in entities:
                    assert (
                        entity["type"] in entity_mentions_by_type
                    ...
                    if rand() <= self.prob:
                        ...
                        start_index_in_doc = sentence.tokens[
                            entity["start"] + changes_counter
                        ].index_in_document
                        ...
                        for i in range(len(token_texts)):
                            tokens.append(
                                model.Token(
                                    text=token_texts[i],
                                    index_in_document=start_index_in_doc + i,
                                    pos_tag=tokenmanager.get_pos_tag([token_texts[i]])[
                                        0
                                    ],
                                    bio_tag="",
                        ...
                        ind_in_sent = tokenmanager.get_index_in_sentence(
                            sentence, entity_text, start_index_in_doc
                        )

                        # search for mentions where the token is listed
                        mentions = tokenmanager.get_mentions(
                            doc, ind_in_sent, sentence_idx
                        )
                        for i in range(len(tokens)):
                            ...
                            if i >= len(entity_text):
                                tokenmanager.create_token(
                                    doc, tokens[i], ind_in_sent + i, mentions[0]
                                )
                                changes_counter += 1
                        if len(entity_text) > len(tokens):
                            ...
                            for i in range(num_of_changes):
                                tokenmanager.delete_token(
                                    doc, start_index_in_doc + len(tokens)
                                )
                                changes_counter -= 1
            sentence_idx += 1
        return doc
    ...
    def extract_entity_mentions_by_type(self, token_sequences, tag_sequences):
        ...
        for sent_tokens, sent_tags in zip(token_sequences, tag_sequences):
            sent_entities = Trafo39Step.extract_entities(self, sent_tokens, sent_tags)
