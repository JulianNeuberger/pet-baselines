...
class Trafo82Step(base.AugmentationStep):
    ...
    def get_params() -> typing.List[typing.Union[params.Param]]:
        return [
            params.BooleanParameter(name="short_to_long"),
            params.BooleanParameter(name="long_to_short"),
            params.FloatParam(name="p", min_value=0.0, max_value=1.0),
            params.IntegerParam(name="bank", min_value=0, max_value=2),
        ]
    ...
    def do_augment(self, doc2: model.Document):
        ...
        if self.bank == 0:
            ...
            contracted_list = Trafo82Step.separate_into_contracted_and_expanded_form(
                self, abb_list, True
            )[0]
            expanded_list = Trafo82Step.separate_into_contracted_and_expanded_form(
                self, abb_list, True
            )[1]
            ...
            for sentence in doc.sentences:
                ...
                for token in sentence.tokens:
                    ...
                    if all_abb != []:
                        ...
                        expanded_list_splitted = expanded_list[
                            all_abb[rand_int] - 1
                        ].split()
                        token_list = []
                        ...
                        mention_id = tokenmanager.get_mentions(
                            doc, index_in_sentence, token.sentence_index
                        )
                ...
                assert (
                    sentence.tokens[len(sentence.tokens) - 1].text == "."
                ), "letzter Token ist kein Punkt 1"
        ...
        if self.long_to_short and random() < self.p:
            ...
            for i in range(len(expanded_double_list)):
                ...
                for sentence in doc.sentences:
                            ...
                            if len(expanded_double_list[i]) > 1:
                                for j in range(1, len(expanded_double_list[i])):
                                    if k + j < len(sentence.tokens):
                                        if (
                                            sentence.tokens[k + j].text
                                            != expanded_double_list[i][j]
                                        ):
                                            is_equal = False
                            if (
                                is_equal
                                and sentence.tokens[k].index_in_document
                                not in index_in_doc_blacklist
                            ):
                                # if is_equal:
                                # print(contracted_list[i])
                                sentence.tokens[k].text = contracted_list[i]
                                sentence.tokens[k].pos_tag = tokenmanager.get_pos_tag(
                                    [contracted_list[i]]
                                )[0]
                                if len(expanded_double_list[i]) > 1:
                                    for j in range(1, len(expanded_double_list[i])):
                                        if k + j < len(sentence.tokens):
                                            if (
                                                k == len(sentence.tokens) - 1
                                                and sentence.tokens[k + j].text == "."
                                            ):
                                                pass
                                            else:
                    ...
                    assert (
                        sentence.tokens[len(sentence.tokens) - 1].text == "."
                    ), "letzter Token ist kein Punkt 2"
        return doc
    ...
    def load(self, bank) -> typing.List[typing.List[str]]:
        ...
        """
        with open(f"./transformations/trafo82/{bank}", "r", encoding="utf-8") as fd:
            content = fd.read()
            line = content.split("\n")
    ...
    def separate_into_contracted_and_expanded_form(
        self, abbreviations: typing.List, case_sensitive: bool
